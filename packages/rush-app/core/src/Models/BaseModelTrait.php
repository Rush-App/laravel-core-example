...
trait BaseModelTrait
{
    ...
    public function createOne(Request $request): array
    {
        ...
        //data can only be written by an authorized user or Admin
        if (!empty($uId = Auth::id()) || !empty($aId = Auth::guard('admin')->id())) {
            if (!empty($uId)) {
                $request->merge(["user_id" => Auth::id()]);
            }
            if (!empty($aId)) {
                $request->merge(["admin_id" => Auth::guard('admin')->id()]);
            }
        } else {
            LoggingService::CRUD_errorsLogging('createOne (1) - 418', Logger::NOTICE);
            return ['error' => true, 'code' => 418, 'message' => __('response_messages.save_error')];
        }
